...
Integrating the results of analyzing genetic dependencies highlighted cellular functions and individual genes that were required or unnecessary for tumors with specific \KRAS{} alleles.
Overall, the analysis demonstrated that the \KRAS{} alleles have distinct genetic interactions linked to their biological differences that can be further investigated as therapeutic targets.
\end{abstract}
...
In addition, there were panel-sequencing data (also called targeted sequencing) available for 3,329 COAD, 4,160 LUAD, 61 MM, and 919 PAAD sample.
More information on the sequencing data is available in the Methods and Supplemental Table XX.
...
% TODO: add information for A146T mutations.
...
The alleles above the diagonal line were predicted to be more frequent than observed, while those below the line were in fact more frequent than predicted by the mutational signatures.
...
The \KRAS{} alleles can be further characterized by the genes they tended to comutate with.
An increased frequency of comutation suggests a synergistic effect whereas a reduced frequency of comutation suggests an inhibitory effect on growth, potentially through a mechanism of collateral lethality.
...
The second group was enriched in genes that encode for proteins in the integrin signaling pathway including \emph{RAP2C}, \emph{MAPK8} (JNK1), \emph{MAPK9} (JNK2), \emph{ITGB5} ($\beta_5$ integrin), \emph{ELMO2}, \emph{PTK2} (FAK) (Supplemental Fig. XX). % TODO
...
This indicated that the genes had patterns in their functionality because they are not randomly distributed throughout the PPIN.
...
A detailed list of all cancer studies can be found in Supplementary Table 1. %TODO
...
Supplementary Table 2 indicates the number of samples per tissue in the GTEx and tumor gene expression data. % TODO
...
\caption{
    ...
    \textbf{b.} Cellular functions enriched in the comutation networks of the \KRAS{} alleles. The size of the dot indicates the p-value of the enrichment and the transparency indicates the number of genes in both the function and the comutation network.
    \textbf{c.} The combined rate of comutation of the genes underlying the enriched functions shown in \textbf{b}. The x-axis indicates the fraction of samples with at least one of the interacting genes mutated in the samples with the indicated \KRAS{} allele (dark) or the other samples (light). The genes were separated into those with an increase (blue, going right) or a reduced (green, going left) rate of comutation with the \KRAS{} allele.
}
...
\caption{
    \textbf{The frequency of mutation of unexpressed genes.} Genes with no detectable level of expression in either cancer samples nor normal tissue samples.
}
